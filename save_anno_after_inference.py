...
with open('path.json', 'r+', encoding="UTF-8") as f :
	...
	for d in tqdm(file_list):   
		...
		for i1 in range(n) :
			...
			mask_coordinate = np.where( pred_masks == True)
			coordinate_item = []
			for idx in range(len(mask_coordinate[0])):
				coordinate_item.append(mask_coordinate[1][idx].item())
				coordinate_item.append(mask_coordinate[0][idx].item())
